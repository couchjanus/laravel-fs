              ...
              <div class="box-tools">
                <div class="input-group input-group-sm">
                  <div class="input-group-btn">
                    <a href="{!! route('admins.create') !!}"><button type="submit" class="btn btn-primary btn-xs pull-right"><i class="fa fa-user-plus"></i> Add New</button></a>
                  </div>
                </div>
              </div>
            </div>
                ...
                @else
                    <table class="table table-hover">
                        ...
                        @foreach($admins as $user)
                            <tr>
                                <td><a href="{!! route('admins.edit', [$user->id]) !!}">{!! $user->name !!}</a></td>
                                <td class="raw-m-hide">{!! $user->email !!}</td>
                                
                                <td class="text-right">
                                    <form method="post" action="{!! url('admins/'.$user->id) !!}">
                                        {!! csrf_field() !!}
                                        {!! method_field('DELETE') !!}
                                        <button class="delete-btn btn btn-xs btn-danger pull-right" type="submit"><i class="fa fa-trash"></i> Delete</button>
                                    </form>
                                    <a class="btn btn-xs btn-default pull-right raw-margin-right-8" href="{!! route('admins.edit', [$user->id]) !!}"><i class="fa fa-pencil"></i> Edit</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
